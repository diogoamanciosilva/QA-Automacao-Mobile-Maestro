...
| BUG-07 | Erro de digitação no texto exibido na tela de acompanhamento do pedido: "Previsão de **etrega**" (faltando o "n" de "entrega") | Cosmético | Finalizar um pedido e visualizar a tela "Pedido realizado" | `E) 1`, `G) 1` — Feature Pedido |
| BUG-08 | Erro de digitação no `id` do botão de adicionar item ao cardápio: `add-item-buttom` (com "m" no lugar de "n") — não impede o funcionamento, mas é uma inconsistência de nomenclatura no código do app | Cosmético | Inspecionar o elemento via `maestro hierarchy` na tela de Cardápio | Confirmado em todos os testes que interagem com o botão de adicionar item |
